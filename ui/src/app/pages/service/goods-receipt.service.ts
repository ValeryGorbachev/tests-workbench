...
import { GoodsReceipt } from 'src/interfaces/goods-receipt';
import { AbstractCrudService } from 'src/services/AbstractCrudService';

@Injectable({ providedIn: 'root' })
export class GoodsReceiptService extends AbstractCrudService<GoodsReceipt> {
    private readonly _subUrl: string;
    constructor(protected override http: HttpClient) {
        super(http);
        this._subUrl = environment.apiBaseUrl + '/goods-receipts';
    }
    protected override entityUrl(): string { return this._subUrl; }

    public find(query = '', page?: number, size?: number): Observable<PageModel<GoodsReceipt>> {
        ...
        return this.http.get<PageModel<GoodsReceipt>>(this._subUrl, { params });
    }
